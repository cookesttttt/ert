...
<script>
  ...
  import {getAllData, deleteData} from '@/api/leaseManagement/equipmentAdmission'
  export default {
    ...
    methods: {
      newClick() {
        this.$router.push({name: 'newAdmission'});
      },
      clickBJ(row) {
        sessionStorage.setItem('editAdmissionId', row.id)
        this.$router.push({name: 'editAdmission'});
      },
      clickOK(row) {
        console.log(row)
