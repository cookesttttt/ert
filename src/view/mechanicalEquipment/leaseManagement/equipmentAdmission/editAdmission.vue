    ...
    <Tabs value="name1">
      <TabPane label="基本信息" name="name1">
        ...
        <Row type="flex" justify="center">
          <Col span="10">
            <Form ref="admissionInformation" :model="admissionInformation" :rules="ruleInline" :label-width="100">
              <Col span="24">
                <FormItem label="入场时间" prop="inDate">
                  <DatePicker v-model="admissionInformation.inDate" type="date" class="width" format='yyyy-MM-dd'
                              :transfer="true"></DatePicker>
                </FormItem>
              </Col>
              <Col span="24">
                <FormItem label="租赁合同编号" prop="rentContractNumber">
                  <Input v-model="admissionInformation.rentContractNumber" class="width" :maxlength="30"></Input>
                </FormItem>
              </Col>
              <Col span="24">
                <FormItem label="设备租赁厂商" prop="equipmentSupplier">
                  <Input v-model="admissionInformation.equipmentSupplier" class="width" :maxlength="30"></Input>
                </FormItem>
              </Col>
              <Col span="24">
                <FormItem label="查验结论" prop="checkResult">
                  <Input v-model="admissionInformation.checkResult" type="textarea" :autosize="{minRows: 3,maxRows:3}"
                         :maxlength="128"
                         class="width"></Input>
                </FormItem>
              </Col>
...
<script>
  ...
  import '../index.less'
  import {getDataEcho, editData, deleteImgData, deleteEquipment} from '@/api/leaseManagement/equipmentAdmission'
  ...
  export default {
    ...
    data() {
      return {
        ...
        ruleInline: {
          inDate: [
            {required: true, type: 'date', message: '请选择入场时间', trigger: 'change'}
          ],
          rentContractNumber: [
    ...
    methods: {
      ...
      clickFinish(state) {
        console.log(this.$refs.admissionList.getInsertRecords())
        console.log(this.files)
        //获取表格数据
        const addTableData = this.$refs.admissionList.getTableData().fullData
        const arryData = []
        ...
        this.$refs.admissionInformation.validate((valid) => {
          console.log(valid)
          if (valid) {
            this.$refs.admissionList.validate(valid1 => {
              if (valid1) {
                const data = {
                  saveStatus:state,
                  id: this.itemId,
                  projectId: JSON.parse(localStorage.getItem("projectId")).id,
                  inDate: formatTime(this.admissionInformation.inDate),
                  rentContractNumber: this.admissionInformation.rentContractNumber,
                  equipmentSupplier: this.admissionInformation.equipmentSupplier,
                  checkResult: this.admissionInformation.checkResult,
                ...
                editData(data).then(res => {
                  ...
                  switch (status) {
                    ...
                    default:
                      this.$Message.success(data.msgContent)
                      this.close()
                      this.$router.push({name: 'equipmentAdmission'});
                      break
                  }
                })
      ...
      //添加表格一行
      insertEvent(row) {
        this.$refs.admissionList.insertAt(row,-1)
      },
      //删除设备
      deleteEquipmentdata() {
        console.log(this.$refs.admissionList.getSelectRecords())
        const selectData = this.$refs.admissionList.getSelectRecords()
        const dataId = {'ids': []}
      ...
      //删除
      confirm() {
        const checkedData = this.$refs.admissionList.getSelectRecords()
        if (checkedData.length !== 0) {
          this.$Modal.confirm({
            ...
            onOk: () => {
              ...
              this.$refs.admissionList.removeSelecteds()
            },
          });
        } else {
        ...
        getDataEcho(this.itemId).then(res => {
          ...
          switch (status) {
            ...
            default:
              ...
              this.admissionInformation.inDate = new Date(data.msgData.inDate)
              this.admissionInformation.rentContractNumber = data.msgData.rentContractNumber
              this.admissionInformation.equipmentSupplier = data.msgData.equipmentSupplier
              this.admissionInformation.checkResult = data.msgData.checkResult
    ...
    activated() {
      this.itemId = sessionStorage.getItem('editAdmissionId')
      this.getEcho()
    }
  }
</script>
