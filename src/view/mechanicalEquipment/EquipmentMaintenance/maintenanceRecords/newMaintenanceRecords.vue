      ...
      <Tabs value="name1" class="bottom20">
        <TabPane label="基本信息" name="name1">
          ...
          <Row type="flex" justify="center">
            <Col span="18">
              <Form ref="newMaintenanceData" :model="newMaintenanceData" :rules="ruleInline" :label-width="100">
                <Col span="12">
                  <FormItem label="设备名称" prop="name">
                    <Input v-model="newMaintenanceData.name" class="width" :maxlength="15"></Input>
                  </FormItem>
                </Col>
                <Col span="12">
                  <FormItem label="设备型号" prop="model">
                    <Input v-model="newMaintenanceData.model" class="width" :maxlength="15"></Input>
                  </FormItem>
                </Col>
                <Col span="12">
                  <FormItem label="设备功率" prop="power">
                    <Input v-model="newMaintenanceData.power" class="width" :maxlength="15"></Input>
                  </FormItem>
                </Col>
                <Col span="12">
                  <FormItem label="维修部位" prop="part">
                    <Input v-model="newMaintenanceData.part" class="width" :maxlength="15"></Input>
                  </FormItem>
                </Col>
                <Col span="12">
                  <FormItem label="维修时间" prop="repairDate">
                    <DatePicker
                      v-model="newMaintenanceData.repairDate"
                      type="date"
                      class="width"
                      format="yyyy-MM-dd"
                      :transfer="true"
                ...
                <Col span="12">
                  <FormItem label="维修类型" prop="repairType">
                    <Select v-model="newMaintenanceData.repairType" class="width">
                      <Option value="大修">大修</Option>
                      <Option value="中修">中修</Option>
                      <Option value="小修">小修</Option>
                      <Option value="事故维修">事故维修</Option>
                      <Option value="诊断定向维修">诊断定向维修</Option>
                    </Select>
                  </FormItem>
                </Col>
                <Col span="12">
                  <FormItem label="维修单位" prop="repairCompany">
                    <Input v-model="newMaintenanceData.repairCompany" class="width" :maxlength="15"></Input>
                  </FormItem>
                </Col>
                <Col span="12">
                  <FormItem label="维修费用" prop="repairCost">
                    <Input v-model="newMaintenanceData.repairCost" class="width" :maxlength="12"></Input>
                  </FormItem>
                </Col>
                <Col span="12">
                  <FormItem label="经办人" prop="transactor">
                    <Input v-model="newMaintenanceData.transactor" class="width" :maxlength="15"></Input>
                  </FormItem>
                </Col>
                <Col span="12">
                  <FormItem label="故障原因" prop="reason">
                    <Input v-model="newMaintenanceData.reason" type="textarea" :autosize="{minRows: 3,maxRows:3}"
                           class="width" :maxlength="255"></Input>
                  </FormItem>
                </Col>
                <Col span="12">
                  <FormItem label="检修结果" prop="result">
                    <Input v-model="newMaintenanceData.result" type="textarea" :autosize="{minRows: 3,maxRows:3}"
                           class="width" :maxlength="255"></Input>
                  </FormItem>
                </Col>
              </Form>
        ...
        <TabPane label="附件">
          ...
          <Upload
            ...
            multiple
            :action="`${baseUrl}/pcm/equipments/repair/bill/attachments`"
          >
...
<script>
  ...
  import {newData,deleteImgData} from '@/api/EquipmentMaintenance/maintenanceRecords'
  ...
  export default {
    ...
    data() {
      return {
        ...
        ruleInline: {
          ...
          part: [
            {required: true, message: '请输入设备维修部位', trigger: 'blur'},
          ],
          repairDate: [
            {required: true, type: 'date', message: '请选择维修时间', trigger: 'change'}
          ],
          repairType: [
    ...
    methods: {
      ...
      clickFinish(state) {
        ...
        this.$refs.newMaintenanceData.validate((v) => {
          if (v) {
            this.$refs.xTable.validate((valid) => {
              if (valid) {
                const data = {
                  'saveStatus': state,
                  'name': this.newMaintenanceData.name,
                  'model': this.newMaintenanceData.model,
                  'power': this.newMaintenanceData.power,
                  'part': this.newMaintenanceData.part,
                  'repairDate': this.newMaintenanceData.repairDate,
                  'repairType': this.newMaintenanceData.repairType,
                  'repairCompany': this.newMaintenanceData.repairCompany,
                  'repairCost': this.newMaintenanceData.repairCost,
                  'transactor': this.newMaintenanceData.transactor,
                  'reason': this.newMaintenanceData.reason,
                  'result': this.newMaintenanceData.result,
                  'equipmentRepairDetailVos': arryData,
                ...
                newData(JSON.parse(localStorage.getItem("projectId")).id, data).then(res => {
                  ...
                  switch (status) {
                    ...
                    default:
                      this.$Message.success(data.msgContent)
                      this.close()
                      this.$router.push({name: 'maintenanceRecords'});
                      break
                  }
                })
              }
