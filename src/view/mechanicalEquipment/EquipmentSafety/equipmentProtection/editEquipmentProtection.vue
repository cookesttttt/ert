...
<script>
  ...
  import {editData,deleteImgData,getEditData} from '@/api/EquipmentSafety/equipmentProtection'
  ...
  import bigImg from '@/components/big-img/Bigimg.vue'
    export default {
        ...
        clickFinish(state) {
          ...
          this.$refs.defendData.validate((valid)=>{
            if(valid){
              ...
              editData(this.itemId, data).then(res => {
                const data = res.data
                let status = isDataSuccess(res)
                switch (status) {
                  case 1:
          ...
          getEditData(this.itemId).then(res => {
            ...
            switch (status) {
              ...
              default:
                ...
                console.log(data)
                 this.defendData.equipments=data.msgData.equipments
                 this.defendData.protectDate=data.msgData.protectDate
                 this.defendData.protectType=data.msgData.protectType
      ...
      activated() {
        this.itemId=sessionStorage.getItem('editEquipmentProtectionId')
        this.getEcho()
      }
    }
</script>
