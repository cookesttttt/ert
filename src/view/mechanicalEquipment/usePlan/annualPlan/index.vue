  ...
  <div class='bigbox'>
    <Card class="left-box">
      <div class="textBox">
        ...
        <p style="width: 170px;float: left">计划名称:{{item.year}}年机械计划</p>
        <div class="right">
      ...
      <el-steps :active="4" style='margin-left: 15px'>
        ...
        <el-step title="步骤 4" description="机械年度使用计划"></el-step>
      </el-steps>
      <div class="bottom20">
        <ButtonGroup>
        ...
        <Upload
          ...
          :action="`${baseUrl}/pcm/equipments/year_plans/import_excel/${isId}`">
        ...
        <Button class="search-btn" type="error" @click="clearancePlan" v-bind:disabled="tableData.length===0" style="margin-left: 10px">&nbsp;清空计划列表</Button>
        ...
        >
          ...
          <vxe-table-column type="index" width="60" min-width="60" fixed="left">
          ...
          <vxe-table-column field="categoryName" title="设备类型" :edit-render="{name: 'input'}"
                            min-width="130"></vxe-table-column>
          ...
          <vxe-table-column field="outDate" title="计划出场时间" :edit-render="{type: 'default'}" min-width="140">
            <template v-slot:edit="{ row }">
              <DatePicker v-model="row.outDate" type="date" format="yyyy/MM/dd"
                          :start-date="new Date(item.year)"
                          :options="options3"
                          :transfer="true"></DatePicker>
            </template>
          ...
          <vxe-table-column field="planCount" title="计划数量" :edit-render="{name: 'input'}" min-width="140"
                            fixed="right"></vxe-table-column>
          <vxe-table-column field="remark" title="备注" :edit-render="{name: 'input'}" min-width="100"
                            fixed="right"></vxe-table-column>
        </vxe-table>
        <!--选择工程部位弹框-->
        ...
        <Modal
          ...
          :mask-closable="false"
        >
          <Table border ref="selection" :columns="columns4" :data="DevicedTableData" height="400"
                 v-if="DevicedTableData.length!==0"></Table>
          <p v-else="DevicedTableData.length===0">请按照前置工作完成机械设备类型</p>
          <div slot="footer">
...
<script>
  ...
  import {
    ...
    clearanceData,
    anewData
  } from '@/api/mechanicalEquipment/annualPlan'
  ...
  export default {
    ...
    data() {
      return {
        ...
        options3: {
          disabledDate(date) {
            return date.valueOf() < new Date(sessionStorage.getItem('idYear')) - 86400000 || date.valueOf() > new Date(parseFloat(sessionStorage.getItem('idYear')) + 1 + '') - 86400000
          }
        },
        checkedData: [],
        checkList: [],
    ...
    methods: {
      ...
      clickLeadingOut() {
        window.open(axios.baseUrl + '/pcm/equipments/year_plans/export_excel/' + JSON.parse(localStorage.getItem('projectId')).id + '/' + parseInt(this.item.year))
      },
      //清空计划
      clearancePlan() {
        this.$Modal.confirm({
          ...
          onOk: () => {
            clearanceData(JSON.parse(localStorage.getItem('projectId')).id, parseInt(this.item.year)).then(res => {
              const data = res.data;
        ...
        this.$Modal.confirm({
          ...
          onOk: () => {
            clearanceData(JSON.parse(localStorage.getItem('projectId')).id, parseInt(this.item.year)).then(res => {
              ...
              anewData(JSON.parse(localStorage.getItem('projectId')).id, parseInt(this.item.year)).then(res => {
                const data = res.data;
                let status = isDataSuccess(res);
                switch (status) {
                  case 1:
          ...
          if (valid) {
            ...
            if (this.isId) {
              ...
              if (editAllData.length !== 0) {
                ...
                editAnnualPlan(editDataAll, JSON.parse(localStorage.getItem('projectId')).id, parseInt(this.item.year)).then(res => {
                  const data = res.data;
                  let status = isDataSuccess(res);
                  switch (status) {
                    case 1:
              ...
              if (newData.length !== 0) {
                ...
                //新增接口
                SaveAnnualPlan(data, JSON.parse(localStorage.getItem('projectId')).id, parseInt(this.item.year)).then(res => {
                  const data = res.data;
                  let status = isDataSuccess(res);
                  switch (status) {
                    case 1:
        ...
        getDeviceData(JSON.parse(localStorage.getItem("projectId")).id).then(res => {
          ...
          switch (status) {
            ...
            case 2:
              ...
              break;
            default:
              console.log(this.DevicedTableData)
      ...
      //  获取表格全部数据
      getData() {
        getAllData(JSON.parse(localStorage.getItem("projectId")).id, parseInt(this.item.year)).then(res => {
          const data = res.data;
          let status = isDataSuccess(res);
          switch (status) {
            case 1:
    ...
    activated() {
      ...
      this.item.year = sessionStorage.getItem('idYear')
      this.initialise()
    },
