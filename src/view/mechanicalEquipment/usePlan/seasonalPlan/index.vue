  ...
  <div class='bigbox'>
    <Card class="left-box">
      <div class="textBox">
        ...
        <p  style="width: 200px;float: left">计划名称:{{item.year}}年第{{item.quarter}}度机械计划</p>
        <div class="right">
      ...
      <div class="bottom20">
        ...
        <Upload
          ...
          :action="`${baseUrl}/pcm/equipments/quarter_plans/import_excel/${isId}`">
          ...
          <vxe-table-column field="inDate" title="计划进场时间"
            ...
            <template v-slot:edit="{ row }">
              <DatePicker v-model="row.inDate"
                          type="date"
                          :start-date="new Date(item.year+'/'+parseFloat(3*item.quarter-2)+'')"
                          :options="options"
                          format="yyyy/MM/dd"
                          :transfer="true"
              ></DatePicker>
...
<script>
  ...
  import {
    ...
    clearanceData,
    anewData
  } from '@/api/mechanicalEquipment/seasonalPlan'
  ...
  export default {
    ...
    data() {
      return {
        ...
        options: {
          disabledDate (date) {
             const routerData=JSON.parse(sessionStorage.getItem('quarter'))
            console.log(routerData)
            let month={}
            switch (routerData.quarter) {
              case 1:
                month={"start":0,'end':3};
                break;
              case 2:
              ...
              case 3:
                ...
                break;
              default:
                month={"start":10,'end':12};
                break;
            }
            return month.end===12?date.valueOf() < new Date(routerData.year+'/'+month.start+'') - 86400000
              || date.valueOf() > new Date(parseFloat(routerData.year+1)+'') - 86400000:
               date.valueOf() < new Date(routerData.year+'/'+month.start+'') - 86400000
              || date.valueOf() > new Date(routerData.year+'/'+parseFloat(month.end+1)+'') - 86400000
          }
        },
        checkedData: [],
        checkList: [],
    ...
    methods: {
      ...
      clickLeadingOut(){
          window.open(axios.baseUrl + '/pcm/equipments/quarter_plans/export_excel/' +
            JSON.parse(localStorage.getItem('projectId')).id+'/'+parseInt(this.item.year)+'/'+parseInt(this.item.quarter))
      },
      //清空计划
      clearancePlan(){
        this.$Modal.confirm({
          ...
          onOk: ()=>{
            clearanceData(JSON.parse(localStorage.getItem('projectId')).id,parseInt(this.item.year), parseInt(this.item.quarter)).then(res => {
              const data = res.data;
        ...
        this.$Modal.confirm({
          ...
          onOk: ()=>{
            clearanceData(JSON.parse(localStorage.getItem('projectId')).id,parseInt(this.item.year), parseInt(this.item.quarter)).then(res => {
              ...
              anewData(JSON.parse(localStorage.getItem('projectId')).id,parseInt(this.item.year), parseInt(this.item.quarter)).then(res => {
                const data = res.data;
                let status = isDataSuccess(res);
                switch (status) {
                  case 1:
        ...
        this.$refs.xTable.validate(valid => {
          if (valid) {
            ...
            if (this.isId) {
              ...
              editSeasonalplan(editDataAll,JSON.parse(localStorage.getItem('projectId')).id,parseInt(this.item.year), parseInt(this.item.quarter)).then(res => {
                const data = res.data;
                let status = isDataSuccess(res);
                switch (status) {
                  case 1:
              ...
              if (newData.length !== 0) {
                ...
                //新增接口
                SaveSeasonalplan(data,JSON.parse(localStorage.getItem('projectId')).id,parseInt(this.item.year), parseInt(this.item.quarter)).then(res => {
                  const data = res.data;
                  let status = isDataSuccess(res);
                  switch (status) {
                    case 1:
      ...
      //  获取表格全部数据
      getData() {
        getAllData(JSON.parse(localStorage.getItem("projectId")).id, parseInt(this.item.year), parseInt(this.item.quarter)).then(res => {
          const data = res.data;
          let status = isDataSuccess(res);
          switch (status) {
            case 1:
    ...
    activated() {
      ...
      this.item=JSON.parse(sessionStorage.getItem('quarter'))
