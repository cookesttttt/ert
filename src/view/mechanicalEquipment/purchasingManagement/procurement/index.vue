    ...
    <Card class="left-box">
      ...
      <div class="bottom20">
        ...
        <Upload
          ...
          :action="`${baseUrl}/pcm/equipments/buy_plan/buy_plans/import_excel/${IsId}`">
          <Button class="search-btn">&nbsp;导入采购计划</Button>
        </Upload>
        <Button class="search-btn" @click="clickLeadingOut" v-bind:disabled="tableData.length===0">&nbsp;导出采购计划</Button>
        <Button class="search-btn" @click="clickEmpty" type="error" v-bind:disabled="tableData.length===0">&nbsp;清空采购计划</Button>
        <br><br>
        <vxe-table
          ref="xTable"
          border
        ...
        >
          ...
          <vxe-table-column field='earliestUseDate' title="最早使用时间" min-width="140"></vxe-table-column>
          <vxe-table-column field="planPrice" title="预计单价(元)" :edit-render="{name: 'input'}" min-width="130"></vxe-table-column>
          <vxe-table-column field="buyDate" title="计划采购时间" :edit-render="{type: 'default'}" min-width="140">
            <template v-slot:edit="{ row }">
              <DatePicker v-model="row.buyDate" type="date" format="yyyy/MM/dd" :transfer="true"></DatePicker>
            </template>
            <template v-slot="{ row }">{{ formatDate(row.buyDate) }}</template>
          </vxe-table-column>
          <vxe-table-column field="planTotalPrice" title="预计总价(元)" :edit-render="{name: 'input'}" min-width="130" fixed="right"></vxe-table-column>
          <vxe-table-column field="planBuyCount" title="计划采购数量" :edit-render="{name: 'input'}" min-width="150" fixed="right"></vxe-table-column>
        </vxe-table>
      </div>
      <Modal v-model="isDevicedModal" @on-cancel="cancel" title="选择机械设备" :mask-closable="false" width="600">
        <Table border ref="selection"
               :columns="columns4"
               :data="VerallPlan" height="500"
               v-if="VerallPlan.length!==0"
        ></Table>
...
<script>
  ...
  import {getData,deleteData,emptyData, importExcel, getVerallPlanData, saveData,editAllData } from '@/api/purchasingManagement/procurement'
  ...
  export default {
    ...
    data() {
      return {
        ...
        validRules: {
          ...
          buyDate: [
            { required: true, type: 'string', message: '请选择计划采购时间' }
          ],
          planBuyCount: [
            { required: true, type: 'number', message: '请输入计划采购数量'},
    ...
    methods: {
      ...
      clickFinish(state){
        ...
        tables.forEach(v=>{
          console.log(v)
          if(v.name===null||v.name===''&&v.planPrice===null||v.planPrice===''&&v.buyDate===null||v.buyDate===''&&v.planTotalPrice===null||v.planTotalPrice===''&&v.planBuyCount===null||v.planBuyCount===''){
            ren.push(v)
          }
        })
        ...
        this.$refs.xTable.validate(v=>{
          if(v){
            ...
            console.log(this.$refs.xTable.getTableData().tableData)
              ...
              const allData = {'buyPlans':[],"saveStatus":state}
              allData.buyPlans=this.$refs.xTable.getRecordset().insertRecords
      ...
      //点击导出Excel
      clickLeadingOut(){
        window.open(axios.baseUrl + '/pcm/equipments/buy_plan/buy_plans/excel/' + JSON.parse(localStorage.getItem('projectId')).id)
      },
      //点击新增
      insertEvent(row) {
      ...
      //获取表格全部数据
      getAllData() {
        getData(JSON.parse(localStorage.getItem('projectId')).id).then(res => {
          const data = res.data
          let status = isDataSuccess(res)
          switch (status) {
            case 1:
      ...
      Preservation(){
        ...
        this.rowData.earliestUseDate = this.selectList.inDate
        this.isDevicedModal = false
      },
      getVerallPlan() {
