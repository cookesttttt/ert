...
import { getEditData , newSuppliesData , newStockingData , getNumber , getEditTableData , getStoreroom } from '@/api/materialSupplies/stocking/stockingCopy'    //获取数据
export default {
  name: 'stockingCopy',
  data() {
    return {
  ...
  methods: {
          ...
          if (this.tableData.length !== 0) {
            this.$refs.xTable.validate(valid2 => {
              if (valid2) {
                ...
                newStockingData(data, window.sessionStorage.getItem('stockingCopyId')).then(res => {
                  const data = res.data
                  console.log(res)
                  let status = isDataSuccess(res)
                  switch (status) {
    ...
    //切换tabs触发获取表格数据
    tabsChange() {
        getEditTableData(window.sessionStorage.getItem('stockingCopyId')).then(
          res => {
            const data = res.data
            let status = isDataSuccess(res)
            switch (status) {
  ...
  activated() {
    ...
    getEditData(window.sessionStorage.getItem('stockingCopyId')).then(res => {
      const data = res.data
      let status = isDataSuccess(res)
      switch (status) {
        case 1:
