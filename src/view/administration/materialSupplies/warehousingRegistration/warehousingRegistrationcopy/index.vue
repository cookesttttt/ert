  ...
  <Card>
    <div class="textBox">
      ...
      <div class="right">
        ...
        <Button  class="search-btn" @click="saveOrder(0)">&nbsp临时保存</Button>
        <Button  class="search-btn" type="primary"  @click="saveOrder(1)">&nbsp完成制单</Button>
      </div>
      <div class="clear"></div>
    </div>
    <div>
...
<script>
  ...
  import {getEditStorageData,getStoreroom,newSuppliesData,newStorageData ,getNumber} from '@/api/materialSupplies/warehousingRegistration/warehousingRegistrationcopy'    //获取数据
  ...
  export default {
              ...
              this.$refs.xTable.validate(valid2=>{
                if (valid2) {
                  ...
                  const data={
                    ...
                    "positionNumber" :this.information.positionNumber,
    ...
    activated() {
      ...
      getEditStorageData(window.sessionStorage.getItem("warehousingRegistrationcopyId")).then(res=>{
        const data = res.data;
        let status = isDataSuccess(res);
        switch (status) {
          case 1:
