...
<script>
  ...
  import {getEditData,newSuppliesData,editData,getEditTableData} from '@/api/materialSupplies/straightInOut/editStraightInOut'    //获取数据
  export default {
            ...
            if(this.tableData.length!==0){
              this.$refs.xTable.validate(valid2=>{
                if (valid2) {
                  ...
                  editData(data,window.sessionStorage.getItem("straightInOutId")).then(res=>{
                    const data = res.data
                    console.log(res)
    ...
    activated() {
      getEditTableData(window.sessionStorage.getItem("straightInOutId")).then(
        res=>{
          const data = res.data;
          let status = isDataSuccess(res);
          switch (status) {
      ...
      getEditData(window.sessionStorage.getItem("straightInOutId")).then(res=>{
        const data = res.data;
        let status = isDataSuccess(res);
        switch (status) {
          case 1:
