        ...
        <TabPane label="基本信息" name="name1">
          ...
          >
            ...
            <FormItem label="出库日期" prop="outDate">
              <DatePicker type="date" v-model="information.outDate"  style="width:400px" format='yyyy-MM-dd'></DatePicker>
            </FormItem>
...
<script>
  ...
  import {getEditData,newSuppliesData,newStockingData ,getNumber,getEditTableData} from '@/api/materialSupplies/straightInOut/straightInOutCopy'    //获取数据
  export default {
    ...
    methods:{
            ...
            if(this.tableData.length!==0){
              this.$refs.xTable.validate(valid2=>{
                if (valid2) {
                  ...
                  newStockingData(data,window.sessionStorage.getItem("straightInOutCopyId")).then(res=>{
                    const data = res.data
                    console.log(res)
                    let status = isDataSuccess(res)
                    switch (status) {
      ...
      //切换tabs触发获取表格数据
      tabsChange(){
          getEditTableData(window.sessionStorage.getItem("straightInOutCopyId")).then(
            res=>{
              const data = res.data;
              let status = isDataSuccess(res);
              switch (status) {
    ...
    activated() {
      this.tabsChange()
      getEditData(window.sessionStorage.getItem("straightInOutCopyId")).then(res=>{
        const data = res.data;
        let status = isDataSuccess(res);
        switch (status) {
          case 1:
          ...
          default:
            ...
            this.information.outDate= new Date(data.msgData.outDate)
            this.information.money=data.msgData.buyer
            this.information.shelfNumber=data.msgData.getOrg
            this.information.operator=data.msgData.getPerson
