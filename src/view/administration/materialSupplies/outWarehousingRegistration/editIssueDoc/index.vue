  ...
  <Card style="height: 100%">
    <div class="textBox">
      ...
      <div class="right">
        ...
        <Button  class="btn" @click="saveOrder(0)" v-bind:disabled="this.information.state===1">&nbsp;临时保存</Button>
...
import { getEditData , getStoreroom , newSuppliesData , editData, getEditTableData } from '@/api/materialSupplies/outWarehousingRegistration/editIssueDoc'    //获取数据
export default {
  name: 'editIssueDoc',
  data(){
    return {
  ...
  methods: {
          ...
          if (this.tableData.length !== 0) {
            this.$refs.xTable.validate(valid2 => {
              if (valid2) {
                ...
                editData(data, window.sessionStorage.getItem('outWarehousingRegistrationId')).then(res => {
                  const data = res.data
                  console.log(res)
                  let status = isDataSuccess(res)
                  switch (status) {
    ...
    //切换tabs触发获取表格数据
    tabsChange() {
        getEditTableData(window.sessionStorage.getItem('outWarehousingRegistrationId')).then(
          res => {
            const data = res.data
            let status = isDataSuccess(res)
            switch (status) {
  ...
  mounted() {
    ...
    this.tabsChange()
    getEditData(window.sessionStorage.getItem('outWarehousingRegistrationId')).then(res => {
      const data = res.data
      let status = isDataSuccess(res)
      switch (status) {
        case 1:
        ...
        default:
          ...
          this.information.arrivalDate = new Date(data.msgData.outDate)
