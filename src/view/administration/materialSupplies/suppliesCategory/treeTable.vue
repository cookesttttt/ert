<template>
  <div style="height: 100%">
    <Card style="height: 100%">
      ...
      <!--<Button class="search-btn" type="primary" @click="Modal">
        新建子类别
      </Button>-->
      <br><br>
      <!--新建子目录弹框-->
...
<script>
  ...
  import { getTableData,newCategory,deleteTableData,editTableData } from '@/api/materialSupplies/suppliesCategory'
    export default {
     name: "treeTable",
     props: ['treeNode', 'dirs'], // dirs表格在组件中的数据  treeNode被选中树的节点
      ...
      methods: {
        ...
        editdocName(){
          ...
          editTableData(this.docName,this.treeNode[0].id).then(msg => {
            if (msg.status === 200) {
              const data = msg.data
              if (data.msgCode === 0) {
        ...
        editDirectory () {
          // 修改非空验证
          if (paramsValidate(this.editData) && /^\d+$/.test(this.editData.orderId)) {
            editTableData(this.editData,this.editData.id).then(msg => {
              if (msg.status === 200) {
                const data = msg.data
                console.log(msg.data)
                if (data.msgCode === 0) {
