    ...
    <Col span="19" style="height: 100%">
      <Card style="height: 100%">
        <div class="search-con">
          ...
          <div class="left">
            <Button v-if="treeNode.length" class="search-btn" type="primary"  :disabled="treeNode[0].isRoot" @click="clickNewQualityCode">新建质量规范</Button>
          </div>
          <div class="right">
...
<script>
  ...
  import { getCategoryTree,allData,deleteTable} from '@/api/qualityControl/qualityspecification'    //获取数据
  export default {
    name: "suppliesName",
    data() {
      return {
            ...
            render: (h, params) => {
              return h('div', [
                h('span', {
                  ...
                  on: {
                    'click': () => {
                    /*  sessionStorage.setItem('detailsQualityCodeId',this.tableData[params.index].id)*/
                      this.$router.push({
                        name: '质量规范详情',
                        query: {
    ...
    methods: {
      clickNewQualityCode(){
        sessionStorage.setItem('qualityCategoryId', JSON.stringify(this.treeNodeId))
        sessionStorage.setItem('qualityCategoryName', JSON.stringify(this.treeNode))
        this.$router.push({path: '/qualityControl/newQualityCode'})
      },
      directoryClick() {
        this.$router.push({path: '/qualityControl/qualityCategory'})
      },
      renderContent(h, {root, node, data}) {
        if(data.title==='质量规范类别'){
          return h('span', {
            style: {
          ...
          return h('span', {
            ...
            on: {
              click: () => {
                ...
                this.treeNode = this.$refs.tree.getSelectedNodes();
                sessionStorage.setItem('qualityCategoryId', JSON.stringify(this.treeNode[0].id))
                sessionStorage.setItem('qualityCategoryName', JSON.stringify(this.treeNode))
                console.log(this.treeNode[0].id)
                this.treeNodeId = this.treeNode[0].id;
                this.pageNum = 1;
        ...
        getCategoryTree(JSON.parse(localStorage.getItem("projectId")).id).then(res=>{
          ...
          switch (status) {
            ...
            default:
              ...
              if(JSON.parse(sessionStorage.getItem('qualityCategoryId'))){
                this.fontData(this.treeData,JSON.parse(sessionStorage.getItem('qualityCategoryId')))
                this.getParent(this.treeData,JSON.parse(sessionStorage.getItem('qualityCategoryId')))
                let getParentData=this.getParent(this.treeData,JSON.parse(sessionStorage.getItem('qualityCategoryId')))
                getParentData.pop()
                getParentData.forEach(v=>{
                  v.expand=true
                })
                this.treeNodeId =JSON.parse(sessionStorage.getItem('qualityCategoryId'))
                sessionStorage.setItem('qualityCategoryName', JSON.stringify(this.treeNode))
                this.getAllData(this.pageNum)
